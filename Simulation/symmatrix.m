...
DHT=[cos(ti+ref) -sin(ti+ref)*cos(alphai) sin(ti+ref)*sin(alphai)      ai*cos(ti+ref) ;
     sin(ti+ref) cos(ti+ref)*cos(alphai)  -cos(ti+ref)*sin(alphai)     ai*sin(ti+ref) ;
     0           sin(alphai)              cos(alphai)                  di             ;
     0           0                        0                            1              ];
    
%1 -> Rotation from the "front" to the desired leg
% T1= subs(DHT, ref, 0);
% T1= subs(T1, ti, pi/6+k*pi/3);
% T1= subs(T1, alphai, 0);
% T1= subs(T1, ai, 0);
% T1= subs(T1, di, 0);

%2 -> Shoulder rotation
T2= subs(DHT, ref, ref_shoulder);
T2= subs(T2, ti, t2);
T2= subs(T2, alphai, 0);
T2= subs(T2, ai, rc);
T2= subs(T2, di, h);
